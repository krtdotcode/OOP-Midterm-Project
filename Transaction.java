...
public class Transaction {
	...
	public double calculateTotal() {
		// naka error kanina kaya eto nalang muna nilagay ko not sure kung eto ba tlga purpose neto
		double totalPrice = 0;
	...
	// service.price() + (service.price * dentist.getCommisionRate) = total price
	// per service
			...
			for (Service service : services) {
				if (service.getDentists().contains(dentist)) { // Corrected for multiple dentists
					receipt.append(String.format("  - %s: PHP %.2f\n", service.getName(),
							(service.getPrice() + (service.getPrice() * dentist.getCommissionRate()))));
